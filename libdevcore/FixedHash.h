...
public:
	...
	explicit FixedHash(CryptoPP::byte const* _bs, ConstructFromPointerType) { memcpy(m_data.data(), _bs, N); }
	...
	explicit operator bool() const { return std::any_of(m_data.begin(), m_data.end(), [](CryptoPP::byte _b) { return _b != 0; }); }
	...
	CryptoPP::byte& operator[](unsigned _i) { return m_data[_i]; }
	/// @returns a particular byte from the hash.
	CryptoPP::byte operator[](unsigned _i) const { return m_data[_i]; }
	...
	CryptoPP::byte* data() { return m_data.data(); }

	/// @returns a constant byte pointer to the object's data.
	CryptoPP::byte const* data() const { return m_data.data(); }

	/// @returns begin iterator.
	auto begin() const -> typename std::array<CryptoPP::byte, N>::const_iterator { return m_data.begin(); }

	/// @returns end iterator.
	auto end() const -> typename std::array<CryptoPP::byte, N>::const_iterator { return m_data.end(); }

	/// @returns a copy of the object's data as a byte vector.
	bytes asBytes() const { return bytes(data(), data() + N); }

	/// @returns a mutable reference to the object's data as an STL array.
	std::array<CryptoPP::byte, N>& asArray() { return m_data; }

	/// @returns a constant reference to the object's data as an STL array.
	std::array<CryptoPP::byte, N> const& asArray() const { return m_data; }
...
private:
	std::array<CryptoPP::byte, N> m_data;		///< The binary data.
};
...
public:
	...
	explicit SecureFixedHash(CryptoPP::byte const* _d, ConstructFromPointerType _t): FixedHash<T>(_d, _t) {}
	...
	CryptoPP::byte const* data() const { return FixedHash<T>::data(); }
