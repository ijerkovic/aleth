...
static const CryptoPP::byte c_rlpMaxLengthBytes = 8;
static const CryptoPP::byte c_rlpDataImmLenStart = 0x80;
static const CryptoPP::byte c_rlpListStart = 0xc0;

static const CryptoPP::byte c_rlpDataImmLenCount = c_rlpListStart - c_rlpDataImmLenStart - c_rlpMaxLengthBytes;
static const CryptoPP::byte c_rlpDataIndLenZero = c_rlpDataImmLenStart + c_rlpDataImmLenCount - 1;
static const CryptoPP::byte c_rlpListImmLenCount = 256 - c_rlpListStart - c_rlpMaxLengthBytes;
static const CryptoPP::byte c_rlpListIndLenZero = c_rlpListStart + c_rlpListImmLenCount - 1;
...
private:
    ...
    template <class _T> void pushInt(_T _i, size_t _br)
    {
        ...
        CryptoPP::byte* b = &m_out.back();
        for (; _i; _i >>= 8)
            *(b--) = (CryptoPP::byte)(_i & 0xff);
    }
