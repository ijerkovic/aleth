...
size_t RLP::length() const
{
    ...
    requireGood();
    size_t ret = 0;
    CryptoPP::byte const n = m_data[0];
    if (n < c_rlpDataImmLenStart)
        return 1;
    else if (n <= c_rlpDataIndLenZero)
...
//			cdebug << "s: " << s << ", p: " << p << ", m_out.size(): " << m_out.size() << ", encodeSize: " << encodeSize << " (br: " << brs << ")";
            ...
            if (s < c_rlpListImmLenCount)
                m_out[p] = (CryptoPP::byte)(c_rlpListStart + s);
            else if (c_rlpListIndLenZero + brs <= 0xff)
            {
                m_out[p] = (CryptoPP::byte)(c_rlpListIndLenZero + brs);
                CryptoPP::byte* b = &(m_out[p + brs]);
                for (; s; s >>= 8)
                    *(b--) = (CryptoPP::byte)s;
            }
            else
...
RLPStream& RLPStream::appendList(bytesConstRef _rlp)
{
    if (_rlp.size() < c_rlpListImmLenCount)
        m_out.push_back((CryptoPP::byte)(_rlp.size() + c_rlpListStart));
    else
...
RLPStream& RLPStream::append(bytesConstRef _s, bool _compact)
{
    ...
    else
    {
        if (s < c_rlpDataImmLenCount)
            m_out.push_back((CryptoPP::byte)(s + c_rlpDataImmLenStart));
        else
...
RLPStream& RLPStream::append(bigint _i)
{
    if (!_i)
        ...
        m_out.push_back((CryptoPP::byte)_i);
    else
    {
        unsigned br = bytesRequired(_i);
        if (br < c_rlpDataImmLenCount)
            m_out.push_back((CryptoPP::byte)(br + c_rlpDataImmLenStart));
        else
        {
            ...
            m_out.push_back((CryptoPP::byte)(c_rlpDataIndLenZero + brbr));
            pushInt(br, brbr);
        }
        pushInt(_i, br);
    }
...
void RLPStream::pushCount(size_t _count, CryptoPP::byte _base)
{
    ...
    m_out.push_back((CryptoPP::byte)(br + _base));	// max 8 bytes.
    pushInt(_count, br);
}
